...
def get_prediction_distribution(test_list, fea_name, SubjectList_tv, values):
    ...
    try:
        ...
        if final_mean[0] <0.5:
            test_list_change = test_list.copy()
            test_list_change.loc[0, fea_name[-1]] = 0.95
            opt_list = get_original_data(test_list_change.copy(), fea_name[2:4])[fea_name]
            opt_value = opt_list[fea_name[-1]].values[0]
        else:
            opt_value = 0
...
## find the keys include count7 in data_dict
key7 = [key for key in data_dict.keys() if 'count7' in key]
group2 = data_dict[key7[0]]

orig_v =[]
reduce_v = []

for i, sub in enumerate(group2):
    print(i)
...
orig_v.to_csv('orig_v_group2.csv')
reduce_v.to_csv('reduce_v_group2.csv')
